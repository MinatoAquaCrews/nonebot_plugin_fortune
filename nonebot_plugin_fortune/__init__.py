...
'''
    The command of "^指定(.*?)签$" is considered to be deprecated in the incoming versions
'''
...
async def _(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    image_file, status = fortune_manager.divine(_theme=None, _spec_path=None, event=event)
    if not status:
...
async def _(event: GroupMessageEvent, state: T_State = Depends(get_user_arg)):
    ...
    if limit == "随机":
        ...
        if not spec_path:
            ...
            image_file, status = fortune_manager.divine(_theme=None, _spec_path=spec_path, event=event)
